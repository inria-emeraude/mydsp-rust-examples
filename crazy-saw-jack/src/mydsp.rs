...
pub fn mydsp_run() -> () {
    ...
    dbg!("Creating Client");
    let (client, _client_status) = Client::new("crazy-saw-jack", ClientOptions::default())
    ...
    //mpsc channel for controling Sawtooth in the callback (thread launched)
    let (tx,rx)= channel::<f32>();

    //Launch sawtooth
    let mut freq = 440.;
    let mut sawtooth = Phasor::new(None,sr);
    ...
    let process_callback = move |_: &Client, ps: &ProcessScope| -> Control {
        for f in 0..(ps.n_frames() as usize) {
            ...
            let v = echo.tick(sawtooth.tick(0.0) * 0.5) as f32;
